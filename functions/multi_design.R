multi_design <- function(plot_on, suffix, Z, locs_index, no_grid, grid_mat, D1, c1, c2, sigma1sq, sigma2sq, Bnow, Bnow_true, W, lambda, coords_all){
  # Perform one-time design at time t
  
  # Given a design of Level 1, choose the best Level 2 design

  GA <- ga_mpi('binary',fitnessfcn,grid_mat, D1, c1, c2, sigma1sq, sigma2sq, Bnow, W, lambda, 
          nBits = no_grid, popSize = 95, maxiter = 2000, run = 30, parallel = "mpi")
  d = GA@solution
  C = -GA@fitnessValue
  ...
  multi_design_result <- list(m1 = nrow(D1), m2 = nrow(D2), coords1 = sampling_locs(D1, locs_index), 
